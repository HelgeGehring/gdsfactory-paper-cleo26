...
Finite-difference time domain electromagnetic simulation is supported through open-source backends including MEEP~\cite{meep} and Luminescent AI, as well as proprietary solvers Tidy3D~\cite{tidy3d} (cloud-based GPU acceleration) and Lumerical FDTD. FEM and multiphysics solvers include Femwell~\cite{femwell} for waveguide mode analysis and thermal simulation, Palace~\cite{palace} for RF and microwave simulations, and MEOW \cite{meow} for eigenmode expansion. TCAD simulation uses DEVSIM~\cite{devsim} for semiconductor device physics and Sentaurus for advanced process simulation.
